 ...
 *
 * html使用方法
 * 	<input type='hidden' id='rows' value='5' name='rows'/>
 ...
 *		var limit = $("#rows").val();
 		...
 		var firtInit=false;
 		try {
			opts = $("#pagination").myPagination('options');
		} catch (e) {
			firtInit = true;
		}
 *
 * var opt = {total:total,items_per_page:limit,num_display_entries:2,current_page:cur,pageList:[],callback:pgCallBack};
 $("#pagination").myPagination(opt);
 *
 *
 * 分页回调
	function pgCallBack(){
		var opts= $("#pagination").myPagination('options');
		$("#page").val(opts.current_page+1);
		$("#rows").val(opts.items_per_page);
		$("ttl_ct").val(0);
		btnForSearch(false);
	}
 *
...
//			}
		...
		if($(".confirmBtn").size() > 1){
			$(".confirmBtn").eq(1).attr("id","confirmBtn1");
			$(".numberInput").eq(1).attr("id","numberInput1")
			$("#confirmBtn1").bind("click", function(e){
				var page_id=$.trim($("#numberInput1").val());
				if(page_id=='' || isNaN(page_id)){
					return false;
				}
				pageSelected(jq,parseInt(page_id)-1,e); 
			});
			$("#numberInput1").bind('keydown',function(e){
				var act = document.activeElement.id;
				var currKey = e.keyCode||e.which||e.charCode;
				if(act=="numberInput1" && currKey == 13){ 
					$("#confirmBtn1").trigger('click');
					return false;
				}
				if (e.stopPropagation) {
					e.stopPropagation();
